...
export const initAccounts = async (keyring: KeyRing) => {
  ...
  const ed25519zek = (
    await getAccountsByNetworkName(NetworkNames.Zekko)
  ).filter(acc => !acc.isTestWallet);
  ...
  if (ed25519zek.length == 0)
    await keyring.saveNewAccount({
      basePath: ZekkoNetworks.Zekko.basePath,
      name: 'Zekko Account 1',
      signerType: ZekkoNetworks.Zekko.signer[0],
      walletType: WalletType.mnemonic,
    });
};
